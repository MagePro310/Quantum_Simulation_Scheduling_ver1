...
def example_problem(big_m: int, timesteps: int, filename: str = "scheduling", jobs: list = None, job_capacities: dict = None, machines: list = None, machine_capacities: dict = None, run_solution_explorer: bool = False):
    """Runs the example problem and saves LP/JSON artifacts.

    Optionally produces a lightweight schedule PDF to mirror a solution
    explorer view.

    Args:
        big_m (int): LP metavariable.
        timesteps (int): LP metavariable.
        filename (str): Base filename for generated artifacts.
        run_solution_explorer (bool): When True, also emit a schedule PDF.
    """
    _problem, job_capacities = _generate_problem(big_m, timesteps, jobs, job_capacities, machines, machine_capacities)
    _, scheduled_jobs, lp_instance = generate_schedule(_problem, SchedulerType.EXTENDED)
    ...
    if run_solution_explorer:
        _render_solution_pdf(scheduled_jobs, filename)


def _render_solution_pdf(jobs: list[JobResultInfo], filename: str) -> None:
    """Render a minimal Gantt-style chart for the produced schedule."""
    if not jobs:
        return

    fig, ax = plt.subplots(figsize=(8, max(2, len(jobs) * 0.5)))
    for idx, job in enumerate(jobs):
        duration = max(0.0, job.completion_time - job.start_time)
        ax.broken_barh(
            [(job.start_time, duration)],
            (idx - 0.3, 0.6),
            facecolors="#4e79a7",
        )
        ax.text(
            job.start_time + duration / 2,
            idx,
            f"{job.name} @ {job.machine}",
            ha="center",
            va="center",
            color="white",
            fontsize=8,
    ...
    ax.set_title("Schedule Overview")
    ax.grid(True, axis="x", linestyle=":", linewidth=0.5)
    plt.tight_layout()
    output_path = f"{filename}_schedule.pdf"
